...
import co.salutary.mobisaude.restful.message.request.PostMessageRequest;
...
import co.salutary.mobisaude.restful.message.response.PostMessageResponse;
...
public class ServiceBroker extends AbstractServiceBroker {
	...
	public PostMessageResponse postMessage(PostMessageRequest request) {
		logger.info(new Object() {}.getClass().getEnclosingMethod().getName());
		logger.info("Data = " + request.getUid() + ":" + request.getPass());
		PostMessageResponse msgResponse = new PostMessageResponse();
		
		try {			
			if (!request.validate()) {
				logger.error(properties.getProperty("co.mobisaude.strings.requestInvalido"));
				msgResponse.setErro(properties.getProperty("co.mobisaude.strings.requestInvalido"));				
			}else{
				msgResponse.setErro("");
			}
		} catch (Exception ex) {
